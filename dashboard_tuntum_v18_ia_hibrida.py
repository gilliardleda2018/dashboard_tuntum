...
URBAN_LOCALIDADES = {
    ...
    "CAMPO VELHO", "VILA BENTO", "TUNTUM DE CIMA", "NOVO MARAJA"
}

def classificar_zona(localidade_key):
    return "Zona Urbana" if localidade_key in URBAN_LOCALIDADES else "Zona Rural"

def montar_resumo_para_ia(base_localidades: pd.DataFrame, base_pontos: pd.DataFrame) -> dict:
    base_localidades = base_localidades.copy()
    base_pontos = base_pontos.copy()

    if "indice_oportunidade" in base_localidades.columns:
        top_oportunidades = base_localidades.sort_values("indice_oportunidade", ascending=False).head(10)
    else:
        top_oportunidades = base_localidades.head(10)

    mais_disputadas = base_localidades.sort_values(
        ["competitividade", "validos"] if "validos" in base_localidades.columns else ["competitividade"],
        ascending=False
    ).head(10)

    mais_abstencao = base_localidades.sort_values("abstencao", ascending=False).head(10)

    resumo = {
        "visao_geral": {
            "localidades_analisadas": int(base_localidades.shape[0]),
            "pontos_votacao_analisados": int(base_pontos.shape[0]),
            "votos_fernando": float(base_localidades["fernando"].sum()) if "fernando" in base_localidades.columns else 0.0,
            "votos_tema": float(base_localidades["tema"].sum()) if "tema" in base_localidades.columns else 0.0,
            "abstencao_total": float(base_localidades["abstencao"].sum()) if "abstencao" in base_localidades.columns else 0.0,
        },
        "top_oportunidades": top_oportunidades[[
            c for c in [
                "zona", "localidade", "local_principal", "locais_correspondentes", "eleitores",
                "abstencao", "fernando", "tema", "competitividade", "indice_oportunidade", "lider"
            ] if c in top_oportunidades.columns
        ]].to_dict(orient="records"),
        "mais_disputadas": mais_disputadas[[
            c for c in [
                "zona", "localidade", "local_principal", "eleitores", "fernando", "tema",
                "competitividade", "indice_oportunidade", "lider"
            ] if c in mais_disputadas.columns
        ]].to_dict(orient="records"),
        "maior_abstencao": mais_abstencao[[
            c for c in [
                "zona", "localidade", "local_principal", "eleitores", "abstencao",
                "fernando", "tema", "indice_oportunidade", "lider"
            ] if c in mais_abstencao.columns
        ]].to_dict(orient="records"),
        "top_pontos_votacao": base_pontos.sort_values(
            "indice_oportunidade", ascending=False
        ).head(10)[[
            c for c in [
                "zona", "ponto_votacao", "localidades_atendidas", "eleitores", "abstencao",
                "fernando", "tema", "competitividade", "indice_oportunidade", "lider"
            ] if c in base_pontos.columns
        ]].to_dict(orient="records"),
    }
    return resumo

def gerar_analise_ia(modelo: str, pergunta_usuario: str, resumo_dados: dict) -> str:
    if OpenAI is None:
        return "A biblioteca openai não está instalada. Rode: pip install openai"

    if not os.environ.get("OPENAI_API_KEY"):
        return "A variável de ambiente OPENAI_API_KEY não está configurada no sistema."

    client = OpenAI()
    instructions = (
        "Você é um analista eleitoral de apoio. "
        "Use apenas os dados fornecidos no JSON. "
        "Não invente números. "
        "Responda em português do Brasil, com linguagem objetiva, clara e executiva. "
        "Estruture a resposta em: síntese geral, principais alertas, oportunidades e ações sugeridas. "
        "Quando falar de localidades, cite os nomes exatamente como vieram nos dados."
    )

    prompt = (
        "Dados do painel eleitoral em JSON:\n"
...
def gerar_analise_local(resumo_dados: dict) -> str:
    vg = resumo_dados.get("visao_geral", {})
    top_op = resumo_dados.get("top_oportunidades", [])
    disputadas = resumo_dados.get("mais_disputadas", [])
    abst = resumo_dados.get("maior_abstencao", [])
    pontos = resumo_dados.get("top_pontos_votacao", [])

    votos_f = float(vg.get("votos_fernando", 0))
    votos_t = float(vg.get("votos_tema", 0))
    total = votos_f + votos_t
    pct_f = (votos_f / total) if total > 0 else 0
    pct_t = (votos_t / total) if total > 0 else 0

    lider_geral = "FERNANDO" if votos_f >= votos_t else "TEMA"
    margem_geral = abs(votos_f - votos_t)

    linhas = []
    linhas.append("### Síntese geral")
    linhas.append(
        f"No conjunto analisado, **{lider_geral}** aparece à frente no total de votos válidos, "
        f"com **{votos_f:,.0f}** votos para Fernando e **{votos_t:,.0f}** para Tema. "
        f"Isso equivale a **{pct_f:.1%}** para Fernando e **{pct_t:.1%}** para Tema, "
        f"com margem absoluta de **{margem_geral:,.0f}** votos."
    )

    if top_op:
        linhas.append("\n### Principais oportunidades")
        for item in top_op[:5]:
            loc = item.get("localidade", "Localidade")
            zona = item.get("zona", "")
            abstencao = item.get("abstencao", 0)
            eleitores = item.get("eleitores", 0)
            comp = item.get("competitividade", 0)
            lider = item.get("lider", "")
            indice = item.get("indice_oportunidade", 0)
            linhas.append(
                f"- **{loc}** ({zona}) — índice **{indice:.3f}**, "
                f"eleitorado de **{eleitores:,.0f}**, abstenção de **{abstencao:,.0f}** "
                f"e competitividade de **{comp:.1%}**. O líder local é **{lider}**."
            )

    if disputadas:
        linhas.append("\n### Principais alertas")
        top_disputadas = ", ".join([str(x.get("localidade", "")) for x in disputadas[:5] if x.get("localidade")])
        linhas.append(
            f"As localidades mais disputadas, e portanto mais sensíveis a mobilização e comunicação, são: **{top_disputadas}**."
        )

    if abst:
        linhas.append("\n### Abstenção e recuperação de voto")
        top_abst = abst[:5]
        for item in top_abst:
            loc = item.get("localidade", "Localidade")
            abstencao = item.get("abstencao", 0)
            lider = item.get("lider", "")
            linhas.append(
                f"- **{loc}** registra **{abstencao:,.0f}** abstenções e deve ser tratado como território prioritário de comparecimento. "
                f"O líder local é **{lider}**."
            )

    if pontos:
        linhas.append("\n### Pontos de votação mais estratégicos")
        for item in pontos[:5]:
            pv = item.get("ponto_votacao", "Ponto")
            locs = item.get("localidades_atendidas", "")
            indice = item.get("indice_oportunidade", 0)
            linhas.append(
                f"- **{pv}** atende **{locs}** e apresenta índice de oportunidade **{indice:.3f}**."
            )

    linhas.append("\n### Ações sugeridas")
    linhas.append(
        "- Priorizar mobilização de comparecimento nas localidades com maior abstenção e maior eleitorado."
    )
    linhas.append(
        "- Reforçar presença de campo e comunicação nas localidades mais competitivas, onde a mudança de poucos votos pode alterar o resultado."
    )
    linhas.append(
        "- Tratar os pontos de votação com maior índice de oportunidade como prioridade operacional de agenda, liderança e ativação."
    )
    return "\n".join(linhas)
...
local = (
    df.groupby("bairro_key", dropna=False)
      .agg(
          ...
          locais_correspondentes=("LOCAL DE VOTAÇÃO", lambda x: " | ".join(sorted(set(map(str, x))))),
...
st.sidebar.subheader("IA no painel")
modelo_ia = st.sidebar.selectbox(
    "Modelo de IA",
    options=["gpt-5.4", "gpt-5.3-instant"],
    index=0
)
pergunta_padrao_ia = st.sidebar.text_area(
    "Pergunta para a IA",
    value="Quais são as principais oportunidades e riscos do cenário atual, considerando urbano, rural, abstenção e competitividade?",
    height=120
)
...
# =========================================================
# VISÃO GERAL DE VOTAÇÃO
# =========================================================
tot_fernando = float(local["fernando"].sum())
tot_tema = float(local["tema"].sum())
tot_validos_geral = tot_fernando + tot_tema

resumo_geral = pd.DataFrame({
    "candidato": ["FERNANDO", "TEMA"],
    "votos": [tot_fernando, tot_tema],
})
resumo_geral["percentual"] = np.where(
    tot_validos_geral > 0,
    resumo_geral["votos"] / tot_validos_geral,
    0
)

st.subheader("Percentual geral de votação")
g1, g2 = st.columns(2)
...
with g1:
    fig_bar_geral = px.bar(
        ...
        y="percentual",
        text="percentual",
        title="Percentual geral dos votos válidos"
    )
    fig_bar_geral.update_traces(texttemplate="%{text:.1%}", textposition="outside")
    fig_bar_geral.update_layout(xaxis_title="Candidato", yaxis_title="Percentual")
    fig_bar_geral.update_yaxes(tickformat=".0%")
...
with g2:
    fig_pizza_geral = px.pie(
        ...
        names="candidato",
        values="votos",
        title="Participação geral na votação"
    )
    fig_pizza_geral.update_traces(textinfo="percent+label")
...
# ANÁLISE COM IA
# =========================================================
st.subheader("Análise estratégica com IA")
st.caption("Este bloco pode usar IA online, IA local ou modo híbrido para interpretar os dados filtrados do painel e gerar uma leitura executiva.")

with st.expander("Como ativar", expanded=False):
    st.markdown(
        "1. Instale a biblioteca oficial: `pip install openai`\n"
        "2. Configure a variável de ambiente `OPENAI_API_KEY` no sistema\n"
        "3. Escolha o modelo na barra lateral e clique no botão abaixo\n4. Se a API estiver sem crédito, use IA local ou IA híbrida"
    )

modo_analise = st.radio(
    "Modo de análise",
    options=["IA online", "IA local", "IA híbrida"],
    index=2,
    horizontal=True
)

if st.button("Gerar análise com IA"):
    resumo_ia = montar_resumo_para_ia(base_filtrada, pontos_votacao[pontos_votacao["zona"].isin(zona_filtro)])
    with st.spinner("Gerando análise..."):
        try:
            if modo_analise == "IA local":
                texto_ia = gerar_analise_local(resumo_ia)
            elif modo_analise == "IA online":
                texto_ia = gerar_analise_ia(modelo_ia, pergunta_padrao_ia, resumo_ia)
            else:
                try:
                    texto_ia = gerar_analise_ia(modelo_ia, pergunta_padrao_ia, resumo_ia)
                except Exception as e_online:
                    texto_ia = (
                        "⚠️ A análise online não pôde ser concluída. "
                        f"Motivo: {e_online}\n\n"
                        + gerar_analise_local(resumo_ia)
                    )
            st.session_state["analise_ia_tuntum"] = texto_ia
        except Exception as e:
            st.session_state["analise_ia_tuntum"] = f"Erro ao gerar análise: {e}"

if "analise_ia_tuntum" in st.session_state:
    st.markdown(st.session_state["analise_ia_tuntum"])
...
for _, row in top5_oportunidade.iterrows():
    st.markdown(
        f"**{row['localidade']}** — locais: **{row['locais_correspondentes']}** | índice **{row['indice_oportunidade']:.3f}** | "
...
with r1:
    ...
    top_oportunidade = ordered.head(top_n)[[
        "localidade", "locais_correspondentes", "eleitores", "abstencao", "fernando", "tema",
        "competitividade", "indice_oportunidade", "lider"
    ]].copy()
    st.dataframe(
...
with r2:
    ...
    mais_disputadas = ordered.sort_values(
        ...
        "localidade", "locais_correspondentes", "fernando", "tema", "validos",
...
with r3:
    ...
    crescimento_fernando = crescimento_fernando[
        ...
        "localidade", "locais_correspondentes", "fernando", "tema", "gap_para_virada",
...
mostrar_tabela_recomendacoes = st.checkbox("Mostrar tabela de recomendações por localidade", value=True)
mostrar_tabela_localidades = st.checkbox("Mostrar tabela estratégica por localidade", value=True)
mostrar_tabela_sem_coord = st.checkbox("Mostrar tabela de localidades sem coordenadas", value=True)

if mostrar_tabela_recomendacoes:
    ...
    st.dataframe(
        reco[[
            "localidade", "locais_correspondentes", "local_principal", "eleitores", "abstencao",
...
if mostrar_tabela_localidades:
    ...
    show = show[[
        "zona", "localidade", "local_principal", "secoes", "locais_correspondentes", "eleitores", "comparecimento",
...
if mostrar_sem_coord and mostrar_tabela_sem_coord:
    st.subheader("Localidades sem coordenadas associadas")
    sem = base_filtrada[base_filtrada["lat"].isna() | base_filtrada["lon"].isna()][[
        "zona", "localidade", "local_principal", "locais_correspondentes", "eleitores", "fernando", "tema", "abstencao"
    ]].copy()
